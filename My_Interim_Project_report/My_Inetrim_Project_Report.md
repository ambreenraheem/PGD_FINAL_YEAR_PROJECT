...
### Progem Name: PGD in Data Science with Artificial Intelligence
### Project Title: Fraud Detection in NGO Project Funding, A Data-Driven Study on Pakistan NGO Dataset
### Batch: VIII
...
•	Visualization: Histograms and bar plots to show distribution of Fraud_Type, counts by NGO_Name and City_Town_Village, and other exploratory plots.
•	Feature scaling: Standardization/normalization applied prior to neural network training (autoencoder).
...
Based on analysis of the uploaded notebook, the following progress has been achieved:
...
•	Modeling:
o	A Keras autoencoder was implemented and trained (the notebook shows training output, e.g., Epoch 1/50 and final logs such as loss and val_loss — an example reported val_loss ≈ 0.0104). Reconstruction errors were computed for each sample.
o	Where labels exist, a supervised classification experiment was performed; the notebook reports a classification table with a weighted average precision/recall/f1 ≈ 0.97 on a test sample (600 records), indicating strong performance on that labeled subset. Details of train/test split and cross-validation should be included explicitly in the final write-up (and can be expanded if requested).
...
•	Autoencoder training: Trained for multiple epochs (example training log shows Epoch 1/50 and final validation loss ~0.0104). Reconstruction errors are small for most normal records; anomalous records had higher reconstruction error and were listed in a “Top Detected Fraudulent NGOs” output.
•	Supervised classification (where labels present): A classification report printed in the notebook reported a weighted average precision, recall and F1 ≈ 0.97 on a labeled test set of size ~600, indicating high performance for the labelled subset. (Full confusion matrix and per-class metrics are in the notebook outputs.)
